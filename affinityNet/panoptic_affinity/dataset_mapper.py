...
from .target_generator import PanopticAffinityTargetGenerator

__all__ = ["PanopticAffinityDatasetMapper"]


class PanopticAffinityDatasetMapper:
    """
    ...
    def from_config(cls, cfg):
        ...
        panoptic_target_generator = PanopticAffinityTargetGenerator(
            ignore_label=meta.ignore_label,
            thing_ids=list(meta.thing_dataset_id_to_contiguous_id.values()),
            ignore_stuff_in_affinities=cfg.INPUT.IGNORE_STUFF_IN_AFFINITIES,
            small_instance_area=cfg.INPUT.SMALL_INSTANCE_AREA,
            small_instance_weight=cfg.INPUT.SMALL_INSTANCE_WEIGHT,
            ignore_crowd=cfg.INPUT.IGNORE_CROWD,
            ...
            instance_loss_weight = cfg.INPUT.INSTANCE_LOSS_WEIGHT,
            num_classes = cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES,
            downsampling_factor = cfg.MODEL.PANOPTIC_AFFINITY.TARGET_DOWNSAMPLING
        )
    ...
    def __call__(self, dataset_dict):
        ...
        pan_seg_gt = rgb2id(utils.read_image(dataset_dict.pop("pan_seg_file_name"), "RGB"))

        # Reuses semantic transform for panoptic labels.
        aug_input = T.AugInput(image, sem_seg=pan_seg_gt.astype(np.float32)) # Convert to float as resizing requires float input.
        _ = self.augmentations(aug_input)
        image, pan_seg_gt = aug_input.image, aug_input.sem_seg.astype('int32')
